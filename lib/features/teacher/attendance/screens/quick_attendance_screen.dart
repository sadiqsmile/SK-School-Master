// features/teacher/attendance/screens/quick_attendance_screen.dart
...
if (selectedClass != null &&
  ...
  Future<void> _loadTodayAttendance() async {
    ...
    final dateKey = "${DateTime.now().year}-"
        "${DateTime.now().month.toString().padLeft(2, '0')}-"
        "${DateTime.now().day.toString().padLeft(2, '0')}";

    final docId = "${classId}_$section";
    ...
    if (doc.exists) {
      ...
      final students = Map<String, String>.from(
        data['students'] ?? {},
      );
      ...
      if (total > 0) {
        todayPercentage = (present / total) * 100;
      }
    }
...
print('Students Found = ${docs.length}');
  ...
  void _updatePercentage() {
    ...
    todayPercentage =
        attendanceMap.isEmpty ? 0 : (present / attendanceMap.length) * 100;
  }
  ...
  Future<void> _saveAttendance() async {
    ...
    final exists = await _service.attendanceExists(
